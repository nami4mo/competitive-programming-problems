...
void solve(){
    ...
    function<ll(ll,ll)> dfs=[&](ll node, ll pare){
        ll d_sum=0;
        ll c_cnt=1;
        for(ll neib:gl[node]){
            if(neib==pare)continue;
            ll cnt=dfs(neib,node);
            d_sum+=(cnt+dsums[neib]);
            c_cnt+=cnt;
        }
        dsums[node]=d_sum;
        cnts[node]=c_cnt;
    ...
    function<void(ll,ll)> dfs2=[&](ll node, ll pare){
        if(pare!=-1){
            ll own_dist=cnts[node]+dsums[node];
            ll own_cnt=cnts[node];
            ll pare_dist=dsums[pare]-own_dist;
            ll pare_cnt=n-own_cnt;
            dsums[node]+=pare_dist+pare_cnt;
        }
        for(ll neib:gl[node]){
            if(neib==pare)continue;
            dfs2(neib, node);
