...
void solve(){
    ll x,y,n; cin >> x >> y >> n;
    // int dp[301][301][601];
    vector<vector<ll>> dp(x+1, vector<ll>(x+y+1,0));
    vector<P> thl(n);
    ...
    REP(i,n){
        vector<vector<ll>> new_dp(x+1, vector<ll>(x+y+1,0));
        auto [t,h] = thl[i];
        REP(j,x+1){
            REP(k,x+y+1){
                new_dp[j][k] = max(new_dp[j][k], dp[j][k]);
                if( j < x && k+t <= x+y ){
                     new_dp[j+1][k+t] = max(new_dp[j+1][k+t], dp[j][k]+h);
                }
            }
        }
        dp = new_dp;
    ...
    ll ans = 0;
    REP(j,x+1)REP(k,x+y+1) ans = max(ans,dp[j][k]);
    cout<<ans<<endl;
}
