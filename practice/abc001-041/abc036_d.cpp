...
P dfs(int node, int pare, vector<vector<int>> &g){
    ll white = 1;
    ll black = 1;
    for( int next_node : g[node] ){
        if(next_node == pare) continue;
        P wbp = dfs(next_node, node, g);
        ll n_white = wbp.first;
        ll n_black = wbp.second;
        white *= (n_white+n_black);
        white%=MOD;
        black *= n_white;
        black%=MOD;
    }
